...
public class DowjonesNews
{
    ...
    private HashSet<DowjonesEventHandler> handlers = new HashSet<DowjonesEventHandler>();
    ...
    void NotifySubscribers(MonitrRespData news)
    {
        ...
        foreach(DowjonesEventHandler h in handlers.ToArray()){
            h(news.Title, news.Link, date);
            DowjonesEvent -= h; // handlers.Remove(h);
        }
    }
}
